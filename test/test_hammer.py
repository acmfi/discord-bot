...
def test_insult():
    assert len(Hammer.get_forbidden_words(SimulatedMessage("Eres un cansaliebres", "Santi", "Normal_User"),
                                          commands_name="")) == 1


def test_no_insult():
    assert len(
        Hammer.get_forbidden_words(SimulatedMessage("Hola, que tal?", "Santi", "Normal_User"), commands_name="")) == 0


def test_several_insults():
    assert len(
        Hammer.get_forbidden_words(SimulatedMessage("Eres muy tonto, imbécil y bacterio", "Santi", "Normal_User"),
                                   commands_name="")) == 3
...
def test_add_existing_word():
    assert Hammer.add_word_blacklist(
        SimulatedMessage("!censor PalabraABanearTest", "Santi", "Junta")) == "La palabra ya estaba baneada"


def test_remove_existing_word():
    assert Hammer.uncensor_word(
        SimulatedMessage("!censor PalabraABanearTest", "Santi", "Junta")) == 'Palabra descensurada correctamente :)'


def test_remove_nonexisting_word():
    assert Hammer.uncensor_word(SimulatedMessage("!censor PalabraABanearTest2", "Santi",
                                                 "Junta")) == "La palabra no está baneada, por lo que no se ha removido"
