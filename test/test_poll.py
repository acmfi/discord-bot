...
from src.extensions.poll import MultipleOptionPollModel, PollModel, Emoji, EMOJIS, PollOption, \
    YesOrNoPollModel, _poll_parser
...
def test_create_yesorno_poll1():
    poll_model = YesOrNoPollModel(None, "Is JS the best language?")
    expected_str = "**Is JS the best language?**\n\n" \
                   ":white_check_mark:   True\n" \
                   ":x:   False"
    expected_poll = YesOrNoPollModel(None, "Is JS the best language?")
    expected_poll.poll_str = expected_str
    assert expected_poll == poll_model